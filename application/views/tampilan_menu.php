<ul class="sidebar-menu">
                                <li class="active">
                                    <a href="<?php echo base_url();?>index.php/home">
                                        <i class="fa fa-dashboard"></i> <span>Dashboard</span>
                                    </a>
                                </li>
                                
                                <li class="treeview">
                                <a href="#">
                                    <i class="fa fa-user"></i>
                                    <span>Anggota</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="<?php echo base_url();?>index.php/anggota"><i class="fa fa-angle-double-right"></i>Data Anggota</a></li>
                                    </ul>
                                </li>

                                <li class="treeview">
                                <a href="#">
                                    <i class="fa fa-book"></i>
                                    <span>Buku</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="<?php echo base_url();?>index.php/buku"><i class="fa fa-angle-double-right"></i> Data Buku</a></li>
                                    </ul>
                                </li>

                                <li class="treeview">
                                <a href="#">
                                    <i class="ion-android-data"></i>
                                    <span>Peminjaman Buku</span>
                                    <i class="fa fa-angle-left pull-right"></i>
                                </a>
                                    <ul class="treeview-menu">
                                        <li><a href="<?php echo base_url();?>index.php/peminjaman"><i class="fa fa-angle-double-right"></i>Data Peminjaman</a></li>
                                       
                                        <li><a href="<?php echo base_url();?>index.php/peminjaman/tambah"><i class="fa fa-angle-double-right"></i>Input Peminjaman</a></li>
                                    </ul>
                                </li>
                                <li>
                                    <a href="tentang.php">
                                        <i class="ion-android-information"></i> <span>Tentang SIPEs</span>
                                    </a>
                                </li>


                            </ul>